...
function main() {
  ...
  const bgBufferGeometry = new THREE.SphereBufferGeometry(bgRadius, bgWidthSegments, bgHeightSegments);
  const bgSphere = new THREE.Mesh(bgBufferGeometry, bgMaterial);
  scene.add(bgSphere);

  // background frame
  const bgFrame = new THREE.Mesh(bgBufferGeometry, wireframeMaterial);
  scene.add(bgFrame);
  ...
  const donutMaterial = new THREE.MeshPhongMaterial({
    color: 0xfffc7c2, shininess: 80, transparent: true, opacity: 0.98,
  });
  const donutBufferGeometry = new THREE.TorusKnotBufferGeometry(donutRadius, tubeRadius, radialSegments, tubularSegments, p, q);
  const donut = new THREE.Mesh(donutBufferGeometry, donutMaterial);
  scene.add(donut);
  ...
  const blobBufferGeometry = new THREE.TorusKnotBufferGeometry(blobRadius, tubyRadius, radialSegments, tubularSegments, r, s);
  ...
  const ballBufferGeometry = new THREE.SphereBufferGeometry(ballRadius, widthSegments, heightSegments);
  const ball = new THREE.Mesh(ballBufferGeometry, ballMaterial);
  ball.position.x = 40;
  scene.add(ball);
  ...
  const purpleStemBufferGeometry = new THREE.CylinderBufferGeometry(purpleStemTopRadius, purpleStemBottomRadius, purpleStemHeight, purpleStemRadialSegments, purpleStemHeightSegments);
  const purpleStem = new THREE.Mesh(purpleStemBufferGeometry, purpleStemMaterial);
  ...
  const darkStemBufferGeometry = new THREE.CylinderBufferGeometry(darkStemTopRadius, darkStemBottomRadius, darkStemHeight, darkStemRadialSegments, darkStemHeightSegments);
  const darkStem = new THREE.Mesh(darkStemBufferGeometry, darkStemMaterial);
  darkStem.position.set(-40, 10, 10);
  scene.add(darkStem);

  // stem with backround texture
  const darkStemTwin = new THREE.Mesh(darkStemBufferGeometry, purpleStemMaterial);
  ...
  const greenStemBufferGeometry = new THREE.ConeBufferGeometry(greenStemTopRadius, greenStemHeight, greenStemRadialSegments, greenStemHeightSegments);
  const greenStem = new THREE.Mesh(greenStemBufferGeometry, greenStemMaterial);
  greenStem.position.set(0, -20, -30);
  scene.add(greenStem);

  // chimney
  const chimneyBoxBufferGeometry = new THREE.BoxBufferGeometry(50, 100, 50, 1, 10, 1);
  const chimneyBox = new THREE.Mesh(chimneyBoxBufferGeometry, wireframeMaterial);
  chimneyBox.position.set(-40, 70, 10);
  chimneyBox.material.color.setHex(0xf2d30c);
  scene.add(chimneyBox);

  const chimneyBoxMembrane = new THREE.Mesh(chimneyBoxBufferGeometry, bgMaterial);
  ...
  const blackBoxBufferGeometry = new THREE.BoxBufferGeometry(10, 10, 10);
  const silentBlackBox = new THREE.Mesh(blackBoxBufferGeometry, blackBoxMaterial);
  silentBlackBox.position.set(120, 175, 50);
  scene.add(silentBlackBox);

  const musicalBlackBox = new THREE.Mesh(blackBoxBufferGeometry, blackBoxMaterial);
  ...
  const fleshStemBufferGeometry = new THREE.TubeBufferGeometry(path, 20, 2, 8, false);
  ...
  const fleshStem = new THREE.Mesh(fleshStemBufferGeometry, fleshStemMaterial);
  ...
  function render(time) {
    ...
    blobsRot.forEach((blob) => {
      blob.rotation.x += Math.random() * Math.tan(2 * Math.PI);
      blob.rotation.y += Math.random() * Math.tan(2 * Math.PI);
      blob.rotation.z += Math.random() * Math.tan(2 * Math.PI);

      blob.position.x += 1 / Math.sinh(2 * Math.PI * time) / 2;
      blob.position.y += Math.sin(2 * Math.PI * time) / 2;
      blob.position.z -= Math.cos(2 * Math.PI * time) / 2;
    });
